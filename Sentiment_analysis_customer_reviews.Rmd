...
To do that first convert the text into dataframe or tibble. Then unnest the text into single words with the help of *unnest_tokens ()* function.
...
```
As you see, a lot of common words are not informative (i.e. and,the,to,is,...). These words are called stop words. So, we can remove the stop words from the tibble with the help of *anti_join()* function and *stop_words* dataset from tm package.
```{r,message=FALSE,warning=FALSE}
# Code
...
### Indentifying the sentiment of the reviews
...
All represent positve and negative words as word cloud:
...
- **Negative words gives us an understanding of things that the company should carefully anaylse to improve the porduct.**
